...
namespace Limbo
{
    public partial class Form1 : Form
    {
        ...
        public bool bethigh = false;
        public string cond = "below";
        ...
        {string resultMultiplier = (99 / response.data.diceRoll.state.result).ToString("0.0000");
            ...
            if (response.data.diceRoll.payoutMultiplier > 0)
            {
        ...
        async Task LimboBet()
        {
            try
            {
                if (running)
                {
                    ...
                    payload.query = "mutation DiceRoll($amount: Float!, $target: Float!, $condition: CasinoGameDiceConditionEnum!, $currency: CurrencyEnum!, $identifier: String!) {\n  diceRoll(\n    amount: $amount\n    target: $target\n    condition: $condition\n    currency: $currency\n    identifier: $identifier\n  ) {\n    ...CasinoBet\n    state {\n      ...CasinoGameDice\n    }\n  }\n}\n\nfragment CasinoBet on CasinoBet {\n  id\n  active\n  payoutMultiplier\n  amountMultiplier\n  amount\n  payout\n  updatedAt\n  currency\n  game\n  user {\n    id\n    name\n  }\n}\n\nfragment CasinoGameDice on CasinoGameDice {\n  result\n  target\n  condition\n}\n";
                    ...
                    else
                    {
                        ...
                        currentWager += response.data.diceRoll.amount;
                        if (response.data.diceRoll.payoutMultiplier > 0)
                        {
                            losestreak = 0;
                            winstreak++;
                        ...
                        last.result = response.data.diceRoll.state.result;
                        last.target = response.data.diceRoll.state.target;
